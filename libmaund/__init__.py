...
def countPerLocation2(df_target_regions, s_seq, len_rgen):
...
def countPerLocation(df_reads, is_rev_match, s_seq, rgen_cridx_beg, rgen_cridx_end, len_rgen):
    a = rgen_cridx_beg
    b = rgen_cridx_end
    df = df_reads.copy()
    df['targetRegion'] = df_reads.seq.str.slice(a,b)    
    return countPerLocation2(df, s_seq, len_rgen)


def run_maund(args, logger):
    t1=time.time()
    ...
    for file_path in f_input:
        ...
        seq_wt_count  = countPerLocation(df_seq_wt, is_rev_match, s_seq, rgen_cridx_beg, rgen_cridx_end, len_rgen)
        seq_wt_ratio = seq_wt_count/wt_subst_count
        samelen_ratio = countPerLocation(df_seq_same_len, is_rev_match, s_seq, rgen_cridx_beg, rgen_cridx_end, len_rgen)/same_len_count
        ...
        #targetRegion : region that corresponds to the target sequence region
            ...
            counts = countPerLocation2(df_wellAligned[df_wellAligned.targetRegion.str.len()==len(RGEN_seq)], s_seq, len_rgen)
